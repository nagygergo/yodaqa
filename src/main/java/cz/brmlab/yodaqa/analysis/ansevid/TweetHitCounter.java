...
public class TweetHitCounter extends JCasAnnotator_ImplBase {
    ...
    public void process(JCas jcas) throws AnalysisEngineProcessException {
        ...
        if (dAnswer.getNumFound() > 0)
            fv.setFeature(AF.TweetAHitsEv, dAnswer.getNumFound());
        if (n > 0) {
            fv.setFeature(AF.TweetHitsEv, n);
            if (dAnswer.getNumFound() > 0)
                fv.setFeature(AF.TweetHitsANormEv, (float) n / dAnswer.getNumFound());
            fv.setFeature(AF.TweetMaxScoreEv, s);
            fv.setFeature(AF.TweetHitsMaxScoreEv, n * s);
        }
